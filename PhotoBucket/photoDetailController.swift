...
class photoDetailController: UIViewController{
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        photoListenerRegistration = photoBucketDocumentManager.shared.startListening(for: photoBucketDocumentId!){ [self] in
            print("TODO")
            self.updateView()
            self.showOrHideEditButton()
            
            self.stackView.isHidden = true
            ImageUtils.load(imageView: photoDisplay, from: photoBucketDocumentManager.shared.photoURL)
        ...
        print("So the url is \(photoBucketDocumentManager.shared.photoURL)")
    }
    
    
    func loadTheCurrentPhoto(){
        ...
        let editAction = UIAlertAction(title: "Submit ", style: UIAlertAction.Style.default){ UIAlertAction in
         ...
         //   self.tphotoBucket?.caption = captionTextField.text!
          //  self.tphotoBucket?.url = URLTextField.text!
            photoBucketDocumentManager.shared.updateCaption(caption: captionTextField.text!)
            self.updateView()
